            ...
            <section className="max-w-3xl container mx-auto my-10">
                ...
                <section className="my-5 flex flex-col gap-5">
                    {reservations.length === 0 ? (
                        <p className="text-gray-500 text-sm">No tienes reservas aún.</p>
                    ) : (
                        reservations.map((r) => (
                            <article key={r.id} className="flex gap-5 p-5 bg-white rounded-lg">
                                <figure>
                                    <img src={`${r.fieldImage}`} alt="Imagen de cancha" className="w-96 h-auto rounded-lg"/>
                                </figure>

                                <section className="w-full">
                                    <header className="flex justify-between items-center ">
                                        <h2 className="text-xl font-semibold">Cancha {r.fieldName}</h2>
                                    ...
                                    <section className="flex justify-between items-center gap-5 flex-1">
                                        <div className="flex gap-3 justify-start items-center">
                                            ...
                                            <span className="flex flex-col font-semibold justify-start"><strong className="text-sm font-medium text-gray-500/50">Fecha</strong>{r.formattedDate}</span>
                                        </div>
                                        <div className="flex gap-3 justify-start items-center">
                                            <span className="text-2xl text-green-500">
                                                <TbClockHour3/>
                                            </span>
                                            <span className="flex flex-col font-semibold justify-start"><strong className="text-sm font-medium text-gray-500/50">Hora</strong> {r.startTime}</span>
                                        </div>
                                        <div className="flex gap-3 justify-start items-center">
                                            <span className="text-2xl text-green-500">
                                                <RiMoneyDollarCircleLine/>
                                            </span>
                                            <span className="flex flex-col font-semibold justify-start"><strong className="text-sm font-medium text-gray-500/50">Costo:</strong> ${r.price}</span>
                                        </div>
                                    </section>
